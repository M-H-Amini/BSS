...
class BSS:
    ...
    def FastICA_Gaussianity(self, max_iters=10, tolerance=1e-8, details=False):
        t0=time.time()
        w=np.random.randn(self.X.shape[0],1)
        w/=alg.norm(w)
        for i in range(max_iters):
            ...
            if details:
                print('New w...',w)
            if np.allclose(last_w,w,atol=tolerance):
                if details:
                    print('Converged in {} iterations!'.format(i))
                break
        if details:
            print('New w...',w)
        ##  Finding all w vectors...
        self.W=self.GramSchmidt(w)
        ##  Recovering sources...
        self.S=self.recoverSources()
        t1=time.time()
...
if __name__=='__main__':
    ...
    f3,mixed3=sm.mixSounds([0,1,2],[-10,0,20],'mix3.wav')
    ...
    #bss.FastICA_Gaussianity(100,details=True)
    ...
    bss.FastICA_Negentropy(details=True)
